  ...
  <div class="container">
    ...
    <div class="container" id="order">
      ...
      <div class="tab-content py-3">
        <div class="tab-pane fade active show" id="ticketTime">
          <form action method="GET">
            ...
            <div class="form-group row">
              ...
              <select name="movies" id="movies" v-on:change="loadMovieDay" class="form-control col-md-10">
                <option id="index" v-bind:selected="index==movie_index" v-bind:value="index"
                  v-for="(movies,index) in movies" :key="index">{{movies.name}}</option>
              </select>
            </div>
            ...
            <div class="form-group row">
              ...
              <select name="days" id="days" v-on:change="loadMovieTime" class="form-control col-md-10">
                <option v-bind:value="index" v-bind:selected="index==day_index" v-for="(day,index) in days"
                  :key="index">{{day.date}} ({{day.weekday}})</option>
              </select>
            </div>
            ...
            <div class="form-group row">
              ...
              <select name="times" id="times" class="form-control col-md-10">
                <option v-bind:value="index" v-bind:selected="index==time_index" v-for="(time,index) in times"
                  :key="index">{{time.time}}</option>
              </select>
            </div>
                ...
                <div class="col-md-3" v-for="(ticket,index) in tickets" :key="index">
                  <div class="card border-secondary my-2">
                    <div class="card-body text-secondary">
                      ...
                      <div class="d-flex">
                        <i class="fa fa-minus-square-o fa-2x p-1" v-on:click="minusTickets(index)"
                          style="cursor:pointer;" aria-hidden="true"></i>
                        <input type="text" pattern="[0-5]" class="form-control text-center" readonly
                          v-model="ticketsNum[index]" />
                        <i class="fa fa-plus-square-o fa-2x p-1" v-on:click="plusTickets(index)" style="cursor:pointer;"
                          aria-hidden="true"></i>
                      </div>
                    </div>
                  </div>
              ...
              <div class="card border-secondary my-2">
                <div class="card-body text-secondary">
                  ...
                  <div class="input-group">
                    <i class="fa fa-minus-square-o fa-2x p-1" v-on:click="minusFoodDrinks(index)"
                      style="cursor:pointer;" aria-hidden="true"></i>
                    <input type="text" pattern="[0-5]" class="form-control text-center" readonly
                      v-model="foodDrinksNum[index]" />
                    <i class="fa fa-plus-square-o fa-2x p-1" v-on:click="plusFoodDrinks(index)" style="cursor:pointer;"
                      aria-hidden="true"></i>
                  </div>
                </div>
              </div>
        ...
        <div>
          <button v-on:click="setProp" class="btn btn-success btn-lg btn-block"
            v-bind:class="{'v-hidden':!showNext}">去選座位</button>
        </div>
      </div>
    </div>
  </div>
